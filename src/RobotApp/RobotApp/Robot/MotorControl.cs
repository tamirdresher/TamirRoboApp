...
namespace RobotApp
{
    ...
    class MotorCtrl
    {
        ...
        private static void CheckSystem()
        {
            ...
            if ((msCurTime - RobotDriver.msLastDirectionTime) > 15000)
            {
                Debug.WriteLine("Safety Stop (CurTime={0}, LastDirTime={1})", msCurTime, RobotDriver.msLastDirectionTime);
                RobotDriver.SetRobotDirection(CtrlCmds.Stop, (int)CtrlSpeeds.Max);
                RobotDriver.FoundLocalControlsWorking = false;
                if ((msCurTime - RobotDriver.msLastMessageInTime) > 12000)
                {
                    NetworkController.NetworkInit(MainPage.serverHostName);
                }

                XBoxController.XboxJoystickCheck();
            }
        ...
        private static void BackupRobotSequence()
        {
            ...
            RobotDriver.SetRobotDirection(CtrlCmds.Backward, (int)CtrlSpeeds.Max);
            MoveMotorsForTime(300);

            // spin 180 degress
            RobotDriver.SetRobotDirection(CtrlCmds.Right, (int)CtrlSpeeds.Max);
